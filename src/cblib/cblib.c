...
cb_errno_t cb_board_init(cb_error_t *err, cb_board_t *board)
{
    ...
    if ((result = cb_hist_stack_init(&board->hist)) != 0)
        return cb_strerror(err, result, "malloc: %s\n", errno);
...
cb_errno_t cb_reserve_for_make(cb_error_t *err, cb_board_t *board, uint32_t added_depth)
{
    ...
    if ((result = cb_hist_stack_reserve(&board->hist, added_depth)) != 0) {
        return cb_strerror(err, result, "realloc: %s\n", errno);
    }
    return 0;
}
                                ...
                                const char *algbr)
{
    ...
    if (strlen(algbr) != 4 && strlen(algbr) != 5) {
        return cb_mkerr(err, KH_EINVAL, "invalid move string length");
    }
    ...
    if (from < 0 || from >= 64 || to < 0 || to >= 64)
        return cb_mkerr(err, KH_EINVAL, "invalid character in move");
    ...
    *mv = CB_INVALID_MOVE;
    ...
    if (*mv == CB_INVALID_MOVE) {
        return cb_mkerr(err, KH_EILLEGAL, "illegal move specified");
    }
    ...
    if (flag & (1 << 15)) { /* Check if this is a promotion. */
        ...
        switch (algbr[4]) {
            ...
            case 'q':
                ...
                break;
            default:
                return cb_mkerr(err, KH_EINVAL, "invlaid character in move");
        }
...
cb_errno_t parse_fen_main(cb_error_t *err, cb_board_t *board, char *fen_main)
{
    ...
    /* Error handling. */
    if (fen_main == NULL)
        return cb_mkerr(err, KH_EINVAL, "missing fen body");
    ...
    while ((c = fen_main[i++]) != '\0') {
        if ('1' <= c && c <= '8') {
            ...
            if (sq < row * 8 || sq > row * 8 + 8)
                return cb_mkerr(err, KH_EINVAL, "row overrun without encountering '/'");
        } else if (c == 'p' || c == 'P') {
            ...
            if (sq % 8 != 0)
                return cb_mkerr(err, KH_EINVAL, "encountered '/' before the end of a row");
            row += 1;
        } else {
            /* Any invalid characters return an error. */
            return cb_mkerr(err, KH_EINVAL, "invalid character in fen body");
        }
    }

    /* Throw errors for the write head not being at the end of the board. */
    if (sq != 64)
        return cb_mkerr(err, KH_EINVAL, "unexpected end to fen body");
...
cb_errno_t parse_fen_turn(cb_error_t *err, cb_board_t *board, char *fen_turn)
{
    /* Error handling. */
    if (fen_turn == NULL)
        return cb_mkerr(err, KH_EINVAL, "missing fen turn");
    ...
    if (*fen_turn == 'w') {
        ...
        return cb_mkerr(err, KH_EINVAL, "unexpected character in fen turn");
    }
...
cb_errno_t parse_fen_rights(cb_error_t *err, cb_board_t *board, char *fen_rights)
{
    ...
    /* Error handling. */
    if (fen_rights == NULL)
        return cb_mkerr(err, KH_EINVAL, "missing fen rights");
    ...
    while ((c = fen_rights[i++]) != '\0') {
        if (c == 'K') {
            ...
            return cb_mkerr(err, KH_EINVAL, "unexpected character in fen rights");
        }
    }
...
cb_errno_t parse_fen_enp(cb_error_t *err, cb_board_t *board, char *fen_enp)
{
    ...
    if (strlen(fen_enp) == 1) {
        if (fen_enp[0] != '-')
            return cb_mkerr(err, KH_EINVAL, "invalid character in fen enp");
        return 0;
    }
    if (strlen(fen_enp) != 2)
        return cb_mkerr(err, KH_EINVAL, "invalid fen enpassant length");
...
cb_errno_t parse_fen_hlfmv(cb_error_t *err, cb_board_t *board, char *fen_hlfmv)
{
    ...
    if (fen_hlfmv != NULL) {
        ...
        if (errno || *endptr != '\0')
            return cb_mkerr(err, KH_EINVAL, "invalid halfmove number");
...
cb_errno_t cb_board_from_fen(cb_error_t *err, cb_board_t *board, char *fen)
{
    ...
    if ((result = cb_reserve_for_make(err, board, 1)) != 0)
        return cb_strerror(err, KH_EABORT, "realloc: %s", errno);
